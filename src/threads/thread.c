...
#define FLOAT_ROUND(A) (A >= 0 ? ((A + (1* (1 << (Q - 1)))) / F ): ((A - (1 * (1 << (Q - 1)))) / F))
...
thread_tick (void) 
{
  ...
  if(thread_mlfqs)
  {
    if(strcmp(t->name, "idle") != 0) //If the thread isnt idle...
    {
...
void
insert_thread_in_ready_list(struct thread* thread_to_insert) {
...
calculate_load_avg(int ready_threads)
{
  ...
  int ready_float = FLOAT_CONST(ready_threads);
  int resultado = FLOAT_ADD(FLOAT_DIV_MIX(FLOAT_MULT_MIX(load_avg, 59), 60), FLOAT_DIV_MIX(FLOAT_ADD_MIX(ready_float, strcmp(thread_current()->name, "idle") == 0?0:1),60));
  return resultado;
}
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  if(thread_mlfqs)
  {
    if(strcmp(t->name,"main") == 0)
      ...
      t->recent_cpu = FLOAT_DIV_MIX(thread_get_recent_cpu(),100);
